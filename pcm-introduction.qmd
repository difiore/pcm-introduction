...
# Optional
## load more info on taxa names
d <- read_csv("Kuderna et al taxa.csv", col_names = TRUE)
d <- d |>
  rowwise() |>
  mutate(Genus = strsplit(label, "_")[[1]][1])

## create a treedata.table object
...
Why are these important?!? Let's simulate *random evolution* of two metric characters on this tree and then look at the relationship between them using regression... (the values for a and sig2 are arbitrary, just to give us a wider range of trait values)
...
par(mar = c(2, 2, 2, 2))
hist(sim$est, breaks = seq(-1.2, 1.2, 0.2))
hist(sim$p, breaks = seq(0,1, by = 0.05))
```

Phylogenetic independent contrasts

Lets pull out seed with the highest estimate from our simulation...
...
First, PIC on a new dataset... how is eye size related to body size?
...
To do PGLS... need to convert our TREE to a correlation structure object.

Different ways to do this, but one simple one is to assume that the correlation between the residual errors of any pairs of species is proportional to the distance on the tree back to the common ancestor of that pair. There are alternative models to derive this correlation structure that imagine more complex forms of evolutionary change.
...
summary(pgls.m)
```
Same results! PIC is a specialized case of PGLS.

More generally, we can include additional covariates.

Let's now see if relative eye size is related to activity pattern, i.e., how is eye size related to both body size and activity pattern?
...
# make a treetable object
# t <- as.treedata.table(tree = pruned_tree, data = d, name_column = "Species")
# alternatively, we can prune during creation of treetable object
# t <- as.treedata.table(tree = tree, data = d, name_column = "Species")
